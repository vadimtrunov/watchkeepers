...
Activated when the orchestrator is invoked as `/loop /rdd --auto …`
(see "Loop continuity" below — `--auto` MUST be wrapped in `/loop`).
The gate prompts are still printed to the transcript for audit, but
the orchestrator emits a deterministic decision instead of waiting
for the operator. Each gate has exactly one auto-`yes` rule;
everything else halts without side effects, mirroring the
conservative posture of interactive halting.

### Loop continuity (`/loop` REQUIRED)

`--auto` MUST be wrapped in `/loop` — invoke as `/loop /rdd --auto
resume` (or `/loop /rdd --auto <id>`). Each `/loop` tick fires a
fresh orchestrator turn that re-enters the skill and resumes from
disk via the State recovery protocol (most recent `TASK-*.md` with
`Status: in-progress`, branch, ROADMAP checkboxes). The
between-tick boundary is the recovery mechanism for silent-exit
after `Agent` returns: if the orchestrator misses its post-`Agent`
text emission inside a tick, the next tick continues from the
disk-recorded last completed phase — work is not lost. Running
`--auto` inline (Phase 1 → Phase 7 back-to-back in a single
orchestrator turn, without `/loop`) was tried on 2026-05-05 and is
forbidden — it hung the runtime under silent-exit with no recovery
path (see `FEEDBACK.md` 2026-05-05). Phase 6 CI polling still uses
an inline bash loop (`sleep 30 && gh pr checks`) within a single
tick; `/loop` is the inter-iteration boundary, not the
intra-iteration one. The skill stops when the auto-rules cannot
resolve a gate (halt — see per-gate rules below) or when the ROADMAP
candidate list is empty (success — print "ROADMAP complete" and
exit).
