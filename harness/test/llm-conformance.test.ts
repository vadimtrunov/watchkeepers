 ...
 * # Cases
 *
 * Two cases register in the suite:
 *
...
for (const c of CASES) {
  describe.skipIf(c.skipReason !== undefined && c.skipReason !== "")(
    ...
    () => {
      it("complete: returns a CompleteResponse with the documented shape (AC4)", async () => {
      ...
      it("stream: emits >=1 delta-shaped event ending in a message_stop (AC5)", async () => {
        const provider = c.factory();
      ...
      it("countTokens: returns a non-negative integer (AC6)", async () => {
        const provider = c.factory();
      ...
      it("reportCost: accepts a Usage record and resolves cleanly (AC7)", async () => {
        const provider = c.factory();
