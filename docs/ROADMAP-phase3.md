...
- [ ] `LLMRouter` Go component taking a task context (Watchkeeper Manifest + turn metadata) and returning the chosen provider.
- [ ] Manifest schema extension: `llm_preferences` sub-object with `cost_tier ∈ {low, medium, high}`, `quality_floor ∈ {fast, balanced, high}`, `speed_target ∈ {best_effort, low_latency}`, and optional `preferred_provider`.
- [ ] Router policy engine: deterministic selector based on preferences + provider-capability matrix (each `LLMProvider` declares tiers/qualities it can serve). Not ML-based; just rules.
- [ ] Fallback chains: per-task configuration of the ordered provider list; on failure (timeout, rate-limit, error), falls through.
- [ ] Per-provider cost tracking: each invocation's token spend tagged with provider id; rollup in Keep cost tables; Watchmaster `report_cost` gains per-provider breakdown.
- [ ] Provider health: router tracks recent error rates per provider; demotes unhealthy providers until recovered.
- [ ] Backwards compatibility: Manifests without `llm_preferences` route to the default (Claude Code), unchanged behavior.
...
- [ ] `llm/codex/` Go package implementing `LLMProvider` against OpenAI's API (or Codex agent endpoints, depending on the product shape at implementation time).
- [ ] Credential handling through the secrets interface; no vendor env vars in core.
- [ ] Capability matrix declared: supported task tiers (`balanced`, `high`), latency profile, cost per 1k tokens.
- [ ] Tool-use translation: OpenAI tool-use schema ↔ our Tool Manifest schema; the harness sees the same interface regardless of provider.
- [ ] Streaming support matching the harness JSON-RPC streaming protocol.
- [ ] Integration test against a mock OpenAI API (contract tests) + smoke test against the real API (gated by env credential).
...
- [ ] `llm/gemma/` Go package speaking the OpenAI-compatible endpoint exposed by vLLM and Ollama (both follow the shape).
- [ ] Inference backend selection: docker-compose service (`ollama` or `vllm`) with Gemma model pre-loaded; operator can swap backends via config.
- [ ] Capability matrix: `cost_tier=low` (self-hosted so no per-token cost, just hardware), `quality_floor=fast` or `balanced` depending on Gemma size loaded.
- [ ] Tool-use translation: Gemma via vLLM/Ollama supports function-calling; wrap to our Tool Manifest schema.
- [ ] Performance guidance in runbook: GPU/CPU requirements by model size, expected throughput; fall back to Codex or Claude Code for high-tier tasks if Gemma insufficient.
- [ ] Hardware-check smoke: `wk llm gemma check` reports inference backend health, model loaded, latency sample.
...
- [ ] SvelteKit app in `/editor/` — served as its own docker-compose service; authenticates against Keep via the same capability-token mechanism as CLI.
- [ ] **Form mode** for non-engineers: guided wizard with sections (Role description, Personality / Language, Capabilities, Immutable core, System prompt — scaffolded with templates).
- [ ] **YAML mode** for engineers: raw YAML editor (Monaco) with JSON-schema-backed validation, auto-complete from the capability dictionary, live linting.
- [ ] Toggle between form ↔ YAML; changes round-trip.
- [ ] Template picker: browse role templates from the catalog (M5), pick one, customize from there.
- [ ] Preview panel: shows the effective system prompt composition, list of capabilities granted with human-readable descriptions, estimated per-day token budget from `llm_preferences`.
- [ ] Submit: creates a Manifest version with `status=proposed`, routes through the existing approval flow (slack-native or git-pr per deployment).
- [ ] Browse existing Watchkeepers: list, view Manifest, view version history (read-only; editing goes through the approval flow).
- [ ] Minimal auth: operator logs in with an operator credential issued by the core (same mechanism as `wk` CLI). No RBAC per our Phase 3 decision — anyone with operator access to the box has full access to the editor.
...
- [ ] Keep schema extension: `role_template` and `role_template_version` tables; content references templates on disk + metadata (tags, description, author, created_at, used_by_count).
- [ ] Template ingestion: platform-shipped templates (from Phase 2 M6 `templates/manifests/`) indexed on platform upgrade; customer-authored templates can be added via editor.
- [ ] Search: tag-based + full-text over description/prompt/capabilities.
- [ ] Template versioning: every published edit creates a new version; existing Watchkeepers pin to the version they spawned from; updating a template does not auto-mutate live Watchkeepers.
- [ ] Usage stats: increment `used_by_count` on each spawn; surface a "top templates this quarter" view in UI.
- [ ] **Customer-authored templates** stay internal to that customer (public cross-org marketplace → Phase 4).
- [ ] Watchmaster tools updated: `template.search(query)`, `template.show(name, version)`, `template.propose_spawn(name, version, customizations)`.
...
- [ ] `gitlab/` Go adapter via `go-gitlab` or direct REST; supports GitLab.com and self-hosted GitLab instances (base URL config).
- [ ] Tool suite (platform Tool Registry):
...
- [ ] Capabilities: `gitlab:read`, `gitlab:write`, `gitlab:review`.
- [ ] Webhook intake symmetric with GitHub (`gitlab.mr_opened`, etc.).
- [ ] Manifest template `code-reviewer-gitlab.yaml` added to catalog.
- [ ] Rate-limit + circuit-breaker on adapter layer.
...
- [ ] `messenger/telegram/` Go adapter via `go-telegram-bot-api` or native.
- [ ] `MessengerAdapter` methods:
...
- [ ] Human identity mapping: `human` row keyed by Telegram user ID (separate column from Slack user ID).
- [ ] Command structure: Telegram's `/command` UX mapped to Watchmaster's DM-chat pattern.
- [ ] K2K over Telegram: supported via bot-to-bot private groups; escalation to lead via the lead's DM.
- [ ] Runbook: Telegram-specific workspace bootstrap, token management, rate-limit guidance.
...
- [ ] Extend `self_tune` built-in tool: now accepts `field=system_prompt` (previously refused at schema layer).
- [ ] **Extra gate: shadow-compare window**. On approval of a `system_prompt` change, the new Manifest does **not** auto-activate. It enters a `shadow_pending` state. For N turns (default 20), the old Manifest stays active and produces the real responses; the new Manifest runs in shadow (receives the same inputs, its responses logged but not sent). Watchmaster compares old vs. shadow each turn: tool call diff, length diff, semantic similarity score from an LLM judge. At the end of the window, Watchmaster posts a comparison report to the lead with per-turn diffs; lead either promotes to active or rejects. Default reject if lead silent for M hours.
- [ ] Immutable core enforcement unchanged — even `system_prompt` edits cannot subvert boundaries declared in `immutable_core.role_boundaries` / `security_constraints` / `escalation_protocols`.
- [ ] Self-tuning quota in M2 applies; `system_prompt` edits count more heavily (default: cost 3 of the daily quota of 3).
- [ ] Rollback via conversational retune (Phase 2 M3) works transparently — rollback creates a new Manifest pinned to a previous version.
...
- [ ] Keep schema: `shared_lesson` table scoped by `role_id`; contains lesson content, source agent, promotion approver, `active_after`, `evidence_log_ref`.
- [ ] Built-in tool `propose_lesson_share(lesson_id)` available to every Watchkeeper. Routes through Watchmaster approval (lead sees the lesson, approves sharing).
- [ ] Harness auto-recall extension: on each turn, harness recalls top-K from Notebook **and** top-K from `shared_lesson` for this agent's role. Shared lessons have their own weight.
- [ ] Shared-lesson lifecycle: lead or admin can `forget` a shared lesson with mandatory reason; `forget` propagates (subsequent retrievals exclude it).
- [ ] Coordinator's daily briefing extended to include a "new shared lessons (past 24h) for your team's roles" section.
- [ ] Shared lessons are **not** promoted to Keep as business knowledge — they remain in the role-scoped namespace. Existing Notebook → Keep promotion (Phase 1 M6 `promote_to_keep`) stays for distinct use case (personal observation → org knowledge).
...
- [ ] Metric-watcher service in Watchmaster scope: subscribes to per-Watchkeeper metrics emitted by the metric harness (Phase 2 M10) — `review_cycle_time`, `tool_error_rate`, `lead_complaint_signal` (explicit `/complaint` slash command), `token_spend_vs_quota`.
- [ ] Threshold config in Manifest (or global defaults): per-metric threshold with direction (e.g., `tool_error_rate > 0.15` for 6 hours).
- [ ] On threshold breach: Watchmaster auto-proposes either (a) revert to the last known-good Manifest version (if the current version is recent), or (b) a specific retune targeting the suspected cause (e.g., high error rate → propose tightening `personality` toward more conservative behavior).
- [ ] Auto-proposal goes through normal approval flow — lead sees a card with: metric breach context (chart snapshot), proposed change, rationale. Approve / Reject / Modify buttons.
- [ ] Auto-proposal cooldown: per Watchkeeper, no more than one auto-proposal per N hours (default 24) to prevent proposal storms.
- [ ] `auto_iteration_proposed` and related events in Keeper's Log.
...
- [ ] **Audit forwarding**: pluggable `AuditSink` interface with implementations for: `stdout-json` (default), `syslog`, `splunk-hec`, `datadog-logs`. Every Keeper's Log entry mirrored to the configured sink with retention-friendly tags (`event_type`, `actor`, `correlation_id`, `severity`).
- [ ] **Retention policy enforcement**: `keepers_log` retention config already in Phase 2 M9; extended here with per-event-type retention (e.g., `auth_event` retained 7y, `debug_event` 30d) as required by SOC2.
- [ ] **GDPR export API**: `POST /gdpr/export` takes a human identity (email / Slack user ID / Telegram user ID); returns a bundle containing: all Keep rows referencing that human, all Manifest versions they approved, all Notebook entries authored in their conversations, all Keeper's Log entries mentioning them. Cryptographic manifest for verifiability.
- [ ] **GDPR delete API**: `POST /gdpr/delete` takes a human identity; purges all personally-identifying fields across Keep tables; tombstones Keeper's Log entries (keeps audit record but replaces payload with hash); archives Notebooks tied to that human's conversations. Immutable `keepers_log.event_type=gdpr_delete_executed` with the purge manifest.
- [ ] CLI: `wk gdpr export <id> --out <bundle>`, `wk gdpr delete <id> --confirm`.
- [ ] Runbook: SOC2 audit trail overview, retention configuration, GDPR workflow, sample audit questions with pointers to evidence.
...
- [ ] Demo deployment: existing Phase 2 Party (Coordinator + GitHub-capable + Confluence-capable) **plus**:
...
- [ ] **Non-engineer acceptance scenario** (the core success metric):
...
- [ ] Scripted demonstration of each new Phase 3 capability in sequence:
...
- [ ] Phase 3 smoke test: `make phase3-smoke` runs a condensed version of the above in CI.
- [ ] Runbook appendix: everything Phase 3-specific — multi-model routing config, Gemma self-hosted inference ops, Telegram workspace setup, Manifest editor UI admin, compliance evidence collection.
