...
- [x] **M7.1** **Auto-inheritance on retire→spawn**: when a new Watchkeeper is spawned with the same `role_id` (derived from template or an explicit field) as a recently retired one, its Notebook is seeded from the predecessor's latest archive **by default**. Opt-out flag at spawn. `notebook_inherited` event logged; lead receives a 24h digest summarizing inherited entries.
