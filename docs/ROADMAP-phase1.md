...
### M9 — Multi-source Tool Registry + self-modification loop [x]
...
- [x] **M9.2** **Resolver + shadow safety** — overlay merge by priority into the effective per-Watchkeeper toolset; same-name conflict resolved by priority with the lower-priority tool marked `tool_shadowed` and a Slack DM to the lead ("Platform now ships `count_open_prs` v1.2.0; your private repo's `count_open_prs` v0.4.1 takes precedence. Review?"); platform-release integration test simulates a platform `main` carrying a same-named tool to assert the private one still wins, the platform one is shadowed, and the warning event fires.
- [x] **M9.3** **Signing + capability dictionary** — `cosign` or `minisign` over tool tarball + manifest with per-source public keys pinned in `config.yaml` (optional in Phase 1, mandatory in Phase 2 for non-local sources; unsigned load refused when enabled); `dict/capabilities.yaml` maps every capability id to a plain-language description (versioned, translation-ready, missing entry is a CI failure). Phase 1 portion shipped via M9.3.a (dictionary + Go loader + bijection + approval-card wiring); M9.3.b (signing) is formally decomposed and deferred to Phase 2 — the M9.1.a `SignatureVerifier` seam is already in place for that ship.
  - [x] **M9.3.a** **Capability dictionary + Go loader + CI bijection + approval-card wiring** — `dict/capabilities.yaml` (versioned schema `{version: 1, capabilities: {<id>: {description: "..."}}}`, translation-ready by virtue of the nested per-id object so future locale keys land without a v2 bump) maps every capability id to a plain-language description; new `core/pkg/capdict/` package (mirrors M9.1.a strict-decoder discipline) houses (1) a strict YAML loader (`yaml.KnownFields(true)`) returning a `Dictionary` value, (2) a frozen `CanonicalCapabilities []string` declaring the closed set, (3) a `Translator(*Dictionary) approval.CapabilityTranslator` factory returning a closure satisfying the M9.4.b card seam; CI completeness gate is a two-way `TestCapabilities_BijectionWithCanonicalSet` (forward: every canonical id has a yaml entry; reverse: every yaml entry is in canonical; `len()` symmetric), mirroring the M9.7 `TestEventTypeVocabulary_RoadmapNames` pattern; approval card production wiring switches the M9.3-pending fallback (`card.go:362-384`) over to the real translator while keeping the nil-translator degradation path for callers without a loaded dictionary.
  - [ ] **M9.3.b** **Signing — cosign / minisign verifier impl + per-source public keys** — deferred to Phase 2 per `docs/lessons/M9.md:290`. The seam already exists at M9.1.a (`SignatureVerifier` interface + `NoopSignatureVerifier` default + `ErrSignatureVerification` sentinel + `source_failed` phase=signature wiring). Phase 2 lands the real verifier and pins per-source public keys in `config.yaml`. Signing is optional in Phase 1 (no acceptance pressure here); Phase 2 makes it mandatory for non-local sources.
- [x] **M9.4** **Authoring + approval + dry-run + repo CI** — `propose_tool(name, purpose, plain_language_description, code_draft, capabilities, target_source ∈ {platform, private})` with mandatory `plain_language_description` (local source never offered to the agent; lead can override `target_source` at approval time); per-deployment `approval_mode ∈ {git-pr, slack-native, both}`; `git-pr` runs the shared CI workflow on the tool repo (typecheck, undeclared-fs/net lint, vitest with coverage, capability-declaration linter, optional signing step on merge), human lead reviews the PR, merge fires a webhook → core re-syncs → hot-loads; `slack-native` (mandatory for `hosted` source) is Watchmaster-as-AI-reviewer running the same gate set in-process and posting an approval card with plain-language description, human-readable capability translations, per-gate pass/fail, heuristic risk level, and `[Approve] [Reject] [Test in my DM] [Ask questions]` buttons; tool manifest declares `dry_run_mode ∈ {ghost, scoped, none}` (ghost stubs writes and records "would have done: X, Y, Z"; scoped runs with broker-injected filters — Slack sends forced to lead's DM, Jira writes to sandbox project; none surfaces an explicit pre-approval warning); shared CI workflow template is published by the platform and consumed by `watchkeeper-tools` and customer-private repos. Decomposed below into four sub-items by lifecycle stage (authoring inputs → approval execution → dry-run runtime → CI template) so each sub-item is shippable end-to-end.
  - [x] **M9.4.a** **Authoring surface — `Manifest.dry_run_mode` field + `propose_tool` agent capability + `approval_mode` config** — `Manifest` schema (M9.1.a) gains a required `dry_run_mode ∈ {ghost, scoped, none}` enum field with strict decoder validation (typo or missing value rejected by the M9.1.a `DisallowUnknownFields` + required-field discipline); a new `core/pkg/approval/` package houses (1) `Mode ∈ {git-pr, slack-native, both}` and `TargetSource ∈ {platform, private}` typed-string enums with `Validate()` (closed-set discriminator pattern from M9.1.a's `SourceKind`); `local` is deliberately absent from `TargetSource` so `propose_tool` cannot resolve to it (lead-override path lands in M9.4.b at approval time); (2) `ProposalInput` validator covering mandatory `plain_language_description`, `target_source` enum membership, non-empty `capabilities`, lower_snake_case `name`; (3) `Proposer.Submit(ctx, ProposalInput) (Proposal, error)` orchestrator that validates inputs, stamps a UUID + `proposed_at`, and emits `TopicToolProposed` on the eventbus with a metadata-only `ToolProposed` payload (proposal id, proposer agent id, target_source, capability count, correlation id — NEVER `code_draft`, NEVER `plain_language_description`, same PII boundary discipline as M9.1.b's `EffectiveToolsetUpdated`); per-deployment `approval_mode` field is added to `core/pkg/toolregistry/` operator config with strict YAML decoding + `Validate()` (`slack-native` mandatory when any `tool_source.kind = hosted`). All approval-execution surface (git-pr webhook, AI-reviewer gates, Slack card, button callbacks) and the dry-run runtime are explicitly deferred to M9.4.b/c. Per-call `IdentityResolver` seam (function shape, M9.1.a pattern) resolves the proposing agent id so per-tenant identity never becomes a process-global static.
  - [x] **M9.4.b** **Approval execution — git-pr webhook + slack-native AI-reviewer + approval card** — `git-pr` flow: signed webhook receiver verifies HMAC, looks up the proposal record, triggers `ToolSyncScheduler.SyncOnce` on the target source, emits `tool_approved` on PR merge; `slack-native` flow: in-process AI-reviewer runs the shared gate set (typecheck stub, undeclared-fs/net lint, vitest stub adapter, capability-declaration linter) + heuristic risk score and posts an approval card with plain-language description, human-readable capability translations (via M9.3's `dict/capabilities.yaml`), per-gate pass/fail, risk level, and `[Approve] [Reject] [Test in my DM] [Ask questions]` buttons; button callbacks fire `tool_approved` / `tool_rejected` events and (for `[Test in my DM]`) trigger the M9.4.c dry-run executor in `scoped` mode forced to the lead's DM.
  - [x] **M9.4.c** **Dry-run runtime executor — `ghost` / `scoped` / `none` modes** — runtime honours `Manifest.DryRunMode` at `InvokeTool` time: `ghost` stubs all write-side broker operations and records a "would have done: X, Y, Z" trace surfaced via the `tool_dry_run_executed` event; `scoped` injects broker-level filters (Slack sends forced to lead's DM, Jira writes to sandbox project) via a per-invocation broker decorator; `none` surfaces an explicit pre-approval warning before the first non-dry-run call. Per-mode tests assert no real write reaches downstream brokers under `ghost`, and a canary asserts the `scoped` Jira destination is the sandbox project ID configured at the deployment level.
  - [x] **M9.4.d** **Shared CI workflow template** — YAML workflow template published in the platform `watchkeeper-tools` repo (consumable from customer-private repos via a single workflow-call reference) running typecheck, undeclared-fs/net lint, vitest with coverage threshold, capability-declaration linter, and optional signing step on merge. Template is versioned (semver-tagged release), pinned by tool repos via `uses: watchkeeper-tools/.github/workflows/tool-ci.yml@vX.Y.Z`, and documented in `docs/operator-runbook.md` so customer-private repos can adopt it with a one-line workflow stub.
- [x] **M9.5** **Local patches + rollback** — `make tools-local-install <folder>` requires a `--reason` field and emits `local_patch_applied` with operator identity, diff hash, and reason; `wk tool rollback <name> --to <version> [--source <source>]` is operator-driven and logged.
- [x] **M9.6** **Hosted ↔ git migration + tool sharing** — `wk tool hosted export <name>` produces a self-contained bundle (manifest + source + tests) that imports cleanly into a fresh git repo; `promote_share_tool` reads `$DATA_DIR/tools/private/<tool>/`, branches and commits in the target (platform `watchkeeper-tools` for broadly-useful tools, customer-owned git repo for customer-IP), opens a PR, and notifies the lead in Slack; auth via baked credentials — PAT (simpler, Phase 1 quickstart default) or GitHub App (scoped, revocable, production-recommended); capability `tool:share` off by default and granted per-Watchkeeper by the lead.
- [x] **M9.7** **Keeper's Log audit surface** — emits `source_synced`, `source_failed`, `tool_proposed`, `tool_ai_review_passed`, `tool_ai_review_failed`, `tool_dry_run_executed`, `tool_approved`, `tool_rejected`, `tool_loaded`, `tool_shadowed`, `tool_retired`, `local_patch_applied`, `signature_verification_failed`, `hosted_tool_stored`, `hosted_tool_exported`, `tool_share_proposed`, `tool_share_pr_opened`, `tool_share_pr_merged`, `tool_share_pr_rejected`.
- [x] **M9.8** **Out of scope for Phase 1** — prompt self-tuning (tool authoring only); multi-Watchkeeper tool sharing via Keeper-to-Keeper (Phase 2). Scope decision recorded; no Phase 1 deliverable.
...
- [x] **Capability-dictionary completeness** — CI fails when a declared capability has no entry in `dict/capabilities.yaml`. Shipped via M9.3.a's two-way bijection test (`core/pkg/capdict/canonical_test.go:TestCapabilities_BijectionWithCanonicalSet`).
...
### M10 — Observability, CLI, operator runbook [x]
...
- [x] **M10.2** **`wk` CLI + Make targets** — `spawn`, `retire`, `list`, `logs <wk>`, `inspect <wk>`, `tail-keepers-log`, `tool list | rollback`, `tool hosted list | show | export`, `tool share <name> --target <repo>`, `tools sources list | status | sync`, `notebook show <wk> | forget <wk> <id> | export <wk> | import <wk> <archive> | archive <wk> | list-archives <wk>`, `personality show <wk> | set <wk>`, `language show <wk> | set <wk>`, `budget show | set`, `approvals pending | inspect <id>` — every command mirrored behind a `make wk CMD="..."` shortcut. Phase 1 portion shipped via the M10.2 atomic PR (#145); CLI contract for every command is pinned NOW with backend-pending subcommands surfacing exit-3 with the follow-up M-id named verbatim in the diagnostic. The exit-3 stubs are formally decomposed below and deferred to Phase 2.
  - [ ] **M10.2.b** **Backend wiring for the stubbed `wk` subcommands** — deferred to Phase 2 per `docs/lessons/M10.md` (M10.2 lesson § "Follow-ups named in CLI diagnostics") and the M10.4 operator runbook (`docs/operator-runbook.md` §Notebook archive backup/restore + §D1). Affected subcommands: `wk notebook archive` + `wk notebook list-archives` (require `archivestore.Storer` config exposed via env — today only the Keep service config plumbs it); `wk tool list`, `wk tool hosted list`, `wk tool hosted show` (require a `toolregistry` index seam + a `hostedexport.List` / `hostedexport.Show` seam); `wk tools sources list | status | sync` (require operator-side bindings to the M9.1 `ToolSyncScheduler`). Every affected stub already names `M10.2.b` in its exit-3 diagnostic so an operator reading the runbook today finds the trail. The CLI flag shapes are frozen by the M10.2 contract test surface; the follow-up lands the seams without re-litigating the operator-facing API. The peer follow-ups `M10.2.c` (approvals + budget set CLI wiring) and `M10.2.d` (server-side `wk logs <wk>` actor filter) remain named in the M10.2 lesson but not formally decomposed here — track via the lesson until an operator asks for them in roadmap.
- [x] **M10.3** **docker-compose + Grafana** — finalize compose stack (core, keep, postgres, watchmaster, sample coordinator, dev Slack socket bridge) and ship a Grafana starter dashboard.
- [x] **M10.4** **Operator runbook + smoke** — runbook covers workspace bootstrap, credential rotation, Keep backup/restore, Notebook archive backup/restore via `ArchiveStore`, runaway-agent incident response, upgrade procedure, and disaster scenarios; `make smoke` reproduces the M7 + M8 + M9 success scenarios against an isolated dev environment.
