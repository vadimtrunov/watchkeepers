...
- [ ] **M3.5.a** (security): plumb `organization_id` through `auth.Claim` so per-tenant filtering is enforceable in keep handlers — cross-tenant gap blocking PR #49 production-ready posture. Requires (1) adding `OrganizationID` field to `auth.Claim` and the capability-broker mint path, and (2) landing per-org RLS policies on `human` and `watchkeeper` tables (keyed off the same scope/org GUC that `knowledge_chunk` uses — migration 005). Split into **a.1** (foundation: `auth.Claim` field + JWT round-trip + `capability.Broker.IssueForOrg`/`ValidateForOrg` sibling methods, no handler changes) and **a.2** (handler wire-up + per-org RLS migration, closes the gap). Until both land, deploy behind a network boundary that admits only authenticated operators.
  ...
  - [ ] **M3.5.a.2** Handler wire-up: `handleInsertHuman` and `handleSetWatchkeeperLead` (plus any other watchkeeper-table mutator) consume `claim.OrganizationID`, reject mismatched request-body org IDs, and (where applicable) call `b.ValidateForOrg`. Land per-org RLS policies on `human` and `watchkeeper` keyed off the watchkeeper.org GUC. Update DEVELOPING.md Security posture once the gap is closed.
